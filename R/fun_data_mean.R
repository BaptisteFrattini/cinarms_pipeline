...
fun_data_mean <- function(meta_data, arms_id){
  ...
  meta_mean <- data.frame(num = c(1:15),
  ...
  meta_mean$immersion_season <- ifelse(meta_mean$arms_name %in% hot_season_arms_imm, "imm_hot", "imm_cold")
  meta_mean$recovery_season <- ifelse(meta_mean$arms_name  %in% hot_season_arms_rec, "rec_hot", "rec_cold")
